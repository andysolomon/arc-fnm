...
> **STATUS: DONE (2026-08-02).** See progress.txt. Implementation notes: resolution is two-step (legal RT body + package decision) on Tactics > Depth Chart after plan lock; four package responses with stated costs; depth chart bench is real drag-and-drop and the panel reflects drags onto RT; illegal o5 reps zero out after lock; Continue is a state-computed guide that turns red on blockers and deep-links to screen+tab. A post-phase UX pass added: answered game-plan cards sink + completion banner, allocated objectives leave the competing list, and the practice plan is a drag kanban.
